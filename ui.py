...
tab2, tab1, tab3, tab4 = st.tabs(["Current Analysis", "Historical Backtest", "Simulation Results", "Portfolio Analyzer"])

from backtest import calculate_performance_metrics, calculate_exact_month_sma, calculate_drawdowns, safe_format, run_backtest_simulation
...
with tab4:
    st.header("Multi-Ticker Portfolio Analyzer")
    st.markdown("Use this tool to run a systematic multi-agent analysis on a basket of tickers.")

    with st.sidebar:
        st.markdown("---")
        st.subheader("Portfolio Parameters")
        port_tickers_input = st.text_input("Tickers (comma separated)", "VTI, AAPL, MSFT, GLD")
        port_sma_months = st.number_input("Analysis Period (months)", min_value=1, max_value=240, value=10, step=1, key="port_months")
        port_timing = st.selectbox("Rebalance Frequency", ["At Signal", "Weekly", "Biweekly", "Monthly", "Bimonthly", "Quarterly"], index=3, key="port_timing")
        port_investment = st.number_input("Initial Capital Per Asset ($)", min_value=1000, max_value=1000000, value=10000, step=1000, key="port_investment")
    ...
    if not port_tickers:
        ...
        # Build Dot Graph dynamically
        dot_str = "digraph AgentWorkflow {\\n"
        dot_str += '  rankdir=LR;\\n'
        dot_str += '  node [shape=box, style="rounded,filled", fillcolor="#2E3440", fontcolor="white", fontname="Helvetica"];\\n'
        dot_str += '  edge [color="#88C0D0", fontname="Helvetica"];\\n'
        dot_str += '  Supervisor_Assign [label="Supervisor\\\\n(Assign)", fillcolor="#4C566A"];\\n'
        dot_str += '  Supervisor_Analyze [label="Supervisor\\\\n(Analyze)", fillcolor="#4C566A"];\\n'
        dot_str += '  Gemini [label="Gemini 2.5 Flash\\\\n(Vertex AI)", fillcolor="#B48EAD", shape=cylinder];\\n'
        
        for t in port_tickers:
            dot_str += f'  Worker_{t} [label="Worker Node\\\\n({t})"];\\n'
            dot_str += f'  Supervisor_Assign -> Worker_{t} [label="Send state"];\\n'
            dot_str += f'  Worker_{t} -> Supervisor_Analyze [label="Return result"];\\n'
            
        dot_str += '  Supervisor_Analyze -> Gemini [label="Prompt"];\\n'
        dot_str += "}"
        ...
        try:
            ...
            st.warning("Graphviz is not installed on this system to render the architecture diagram, but the agent will still run.")

        if st.button("Analyze Portfolio", type="primary", use_container_width=True):
            from portfolio_agent import build_portfolio_graph
            graph = build_portfolio_graph()
            
            import datetime
            initial_state = {
                "tickers": port_tickers,
                "start_date": st.session_state.get("hist_start", datetime.date(2015, 12, 31)),
                "end_date": st.session_state.get("hist_end", datetime.date.today()),
                "sma_months": port_sma_months,
                "timing": port_timing,
                "initial_investment": port_investment,
                "worker_results": [],
                "analysis": ""
            }
                ...
                for event in graph.stream(initial_state):
                    for node_name, state_update in event.items():
                        ...
                        if "worker_results" in state_update:
                            for res in state_update["worker_results"]:
                                st.write(f"  - Retrieved backtest for: **{res['ticker']}**")
                            final_worker_results.extend(state_update["worker_results"])
                        if "analysis" in state_update:
                            final_analysis = state_update["analysis"]
                        if "allocations" in state_update:
                            final_allocations = state_update["allocations"]
                            
                status.update(label="Portfolio Analysis Complete!", state="complete", expanded=False)

            st.markdown("### Quantitative Results")
            results_data = []
            for wr in final_worker_results:
                ticker = wr["ticker"]
                res = wr["result"]
                if "error" not in res:
                    results_data.append({
                        ...
                        "Total Return (%)": res.get("total_return", 0) * 100,
                        "CAGR (%)": res.get("cagr", 0) * 100,
                        "Sharpe Ratio": res.get("sharpe", 0),
                        "Sortino Ratio": res.get("sortino", 0)
                    })
            ...
            if results_data:
                ...
                if final_allocations:
                    alloc_dict = final_allocations
                    tickers = list(alloc_dict.keys())
                    values = list(alloc_dict.values())
                    st.markdown("### AI Recommended Portfolio Allocation")
                    ...
                    st.markdown("### Final Portfolio Value Distribution (Placeholder)")
                    fig = go.Figure(data=[go.Pie(labels=df["Ticker"], values=df["Final Value"], hole=.3)])
                    fig.update_layout(template="plotly_dark", margin=dict(t=0, b=0, l=0, r=0))
                    st.plotly_chart(fig, use_container_width=True)
            
            st.markdown("### Qualitative Analysis (Gemini 2.5 Flash)")
            st.markdown(final_analysis)
            
            st.caption("Powered by LangGraph Supervisor-Worker Architecture + Gemini 2.5 on Vertex AI (GCP)")